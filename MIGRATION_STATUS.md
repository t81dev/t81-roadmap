# Migration Status Board

Snapshot time (UTC): 2026-02-08T22:44:47Z.

Legend: `green` = contract marker aligned, `yellow` = marker missing, `red` = marker drift.

| Repo | Status | Owner | Last Verified SHA | Blocking Issue |
| --- | --- | --- | --- | --- |
| `t81-vm` | green | @t81dev | `ae823658da63` | none |
| `t81-lang` | green | @t81dev | `93a47188d3a6` | none |
| `t81-python` | green | @t81dev | `33e300607e13` | none |
| `t81-docs` | green | @t81dev | `98c90eb3c3d2` | none |
| `t81-examples` | green | @t81dev | `fc578305e229` | none |
| `ternary-tools` | green | @t81dev | `e8fbb01cde5f` | none |
| `ternary-pager` | green | @t81dev | `d838f0978243` | none |
| `t81-benchmarks` | green | @t81dev | `c82a0f700a96` | none |
| `t81-roadmap` | green | @t81dev | `bbee1d84e5db` | none |

Source of truth: `ECOSYSTEM_RELEASE_MANIFEST.json`.
Blocker map source: `MIGRATION_BLOCKERS.json`.
